...
if(lbwpsOptions.history==1){if(!isFirstHashUpdate){history.back()}
...
let pid=lightbox.pswp.currSlide.index+1;let newHash='gid='+gid+'&pid='+pid;let newURL=window.location.href.split('#')[0]+'#'+newHash;if(supportsPushState){if(isFirstHashUpdate){history.pushState('',document.title,newURL)}else{history.replaceState('',document.title,newURL)}}else{if(isFirstHashUpdate){window.location.hash=newHash}else{window.location.replace(newURL)}}
isFirstHashUpdate=!1})}
lightbox.init();if(lbwpsOptions.hide_scrollbars==='1'){hideScrollbar()}
lightbox.loadAndOpen(index);window.addEventListener('hashchange',()=>{let hashData=photoswipeParseHash();if(!hashData.gid&&!hashData.pid){if(lightbox.pswp){lightbox.pswp.close();isFirstHashUpdate=!0}}else{isFirstHashUpdate=!1;clickElement(hashData.gid,hashData.pid)}})};let clickElement=function(gid,pid){let elements;if(gid==1){elements=document.querySelectorAll('a[data-lbwps-width]:not([data-lbwps-gid])')}else{elements=document.querySelectorAll('a[data-lbwps-width][data-lbwps-gid="'+gid+'"]')}
