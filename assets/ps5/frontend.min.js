import PhotoSwipeLightbox from './lib/photoswipe-lightbox.esm.min.js';import PhotoSwipeDynamicCaption from './dynamic-caption/photoswipe-dynamic-caption-plugin.esm.min.js';let lbwpsInit=function(domUpdate){const fullscreenAPI=getFullscreenAPI();if(!domUpdate){document.addEventListener('click',(event)=>{if(event.target.parentNode.getAttribute('data-lbwps-width')){event.preventDefault();openPhotoSwipe(!1,0,event.target.parentNode,!1,'');return}
...
galleryItems.push({src:element.getAttribute('href'),msrc:msrc,width:element.getAttribute('data-lbwps-width'),height:element.getAttribute('data-lbwps-height'),title:title,exif:element.getAttribute('data-lbwps-exif'),el:element,tabindex:tabindex})}}
...
return[galleryItems,parseInt(index,10)]};let openPhotoSwipe=function(element_index,group_index,element,fromURL,returnToUrl){let id=1,items,index;if(element!=null){id=element.getAttribute('data-lbwps-gid')}else{id=group_index}
...
options.spacing=lbwpsOptions.spacing/100;options.loop=lbwpsOptions.loop==='1';options.wheelToZoom=lbwpsOptions.wheelmode==='zoom';options.pinchToClose=lbwpsOptions.pinchtoclose==='1';options.closeOnVerticalDrag=lbwpsOptions.close_on_drag==='1';options.clickToCloseNonZoomable=!0;const lightbox=new PhotoSwipeLightbox(options);if(lbwpsOptions.hide_scrollbars==='1'){lightbox.on('destroy',()=>{showScrollbar()})}
if(lbwpsOptions.show_fullscreen==='1'){const fullscreenSVG='<svg aria-hidden="true" class="pswp__icn" viewBox="0 0 32 32" width="32" height="32">'+'<use class="pswp__icn-shadow" xlink:href="#pswp__icn-fullscreen-exit"/>'+'<use class="pswp__icn-shadow" xlink:href="#pswp__icn-fullscreen-request"/>'+'<path id="pswp__icn-fullscreen-request" transform="translate(4,4)" d="M20 3h2v6h-2V5h-4V3h4zM4 3h4v2H4v4H2V3h2zm16 16v-4h2v6h-6v-2h4zM4 19h4v2H2v-6h2v4z" /></g>'+'<path id="pswp__icn-fullscreen-exit" style="display:none" transform="translate(4,4)" d="M18 7h4v2h-6V3h2v4zM8 9H2V7h4V3h2v6zm10 8v4h-2v-6h6v2h-4zM8 15v6H6v-4H2v-2h6z"/>'+'</svg>';lightbox.on('uiRegister',function(){lightbox.pswp.ui.registerElement({name:'fullscreen-button',title:'Toggle fullscreen',order:9,isButton:!0,html:fullscreenSVG,onClick:(event,el)=>{toggleFullscreen()}});lightbox.pswp.events.add(document,'keydown',(e)=>{if(e.keyCode==70){toggleFullscreen();e.preventDefault()}})});lightbox.on('close',()=>{if(fullscreenAPI&&fullscreenAPI.isFullscreen()){fullscreenAPI.exit()}})}
if(lbwpsOptions.show_caption){const captionPlugin=new PhotoSwipeDynamicCaption(lightbox,{type:lbwpsOptions.caption_type,captionContent:(slide)=>{let caption=slide.data.el.getAttribute('data-lbwps-caption');if(!caption){caption=''}
let exif=slide.data.el.getAttribute('data-lbwps-exif');if(!exif){exif=''}
return'<span class="pswp__caption">'+caption+'</span>'+'<span class="pswp__caption__exif">'+exif+'</span>'}})}
lightbox.init();if(lbwpsOptions.hide_scrollbars==='1'){hideScrollbar()}
lightbox.loadAndOpen(index)};function getFullscreenAPI(){let api;let enterFS;let exitFS;let elementFS;let changeEvent;let errorEvent;if(document.documentElement.requestFullscreen){enterFS='requestFullscreen';exitFS='exitFullscreen';elementFS='fullscreenElement';changeEvent='fullscreenchange';errorEvent='fullscreenerror'}else if(document.documentElement.webkitRequestFullscreen){enterFS='webkitRequestFullscreen';exitFS='webkitExitFullscreen';elementFS='webkitFullscreenElement';changeEvent='webkitfullscreenchange';errorEvent='webkitfullscreenerror'}
...
return api}
function toggleFullscreen(){if(fullscreenAPI){if(fullscreenAPI.isFullscreen()){fullscreenAPI.exit();setTimeout(function(){document.getElementById('pswp__icn-fullscreen-exit').style.display='none';document.getElementById('pswp__icn-fullscreen-request').style.display='inline'},300)}else{fullscreenAPI.request(document.querySelector(`.pswp`));setTimeout(function(){document.getElementById('pswp__icn-fullscreen-exit').style.display='inline';document.getElementById('pswp__icn-fullscreen-request').style.display='none'},300)}}}
